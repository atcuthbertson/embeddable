  ...
  function showPane(){
    ...
    setTimeout(arrowRight,100)
      ...
      if(oldIE){
        for(;i<j;i++){
          ...
          else fx.animateProperty({node:movers[i], duration:300, properties:{marginRight:ieMargin}}).play();
        }
      }else{
        for(;i<j;i++)
	...
	function hidePane(){
		...
		if(oldIE){
			for(;i<j;i++){
				if(movers[i] === rightPane)
					fx.animateProperty({node:movers[i], duration:250, properties:{marginRight:-(ieMargin)}}).play();
